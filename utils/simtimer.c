 ...
 *  This is code slightly altered from Michael Leverington's SimpleTimer code.
 *  Made to function with the sim02 code given here.
 *
...
void *runTimer( void *msTime )
   {
    int *msTimePtr = (int *)msTime;
    int milliSeconds = *msTimePtr;
...
double accessTimer( int controlCode, char *timeStr )
   {
    static enum Bool running = False;
